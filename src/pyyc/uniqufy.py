...
class Uniqufy():
    ...
    def visit(self, n, parent = None):
        if isinstance(n, Module):
            ...
            return Call(Name(n.func.id, n.func.ctx), [self.visit(arg) for arg in n.args], [])
...
def uniquify_vars(n, print_asts):
    ...
    uniquifed_ast = uniquify.get_uniqufied_ast(n)
    if print_asts:
        print("UNiquifed AST:")
        print(ast.dump(uniquifed_ast, indent=4))
    unify = Unify()
    unified_ast = unify.get_unified_ast(uniquifed_ast)
    if print_asts:
        print("Unified AST:")
